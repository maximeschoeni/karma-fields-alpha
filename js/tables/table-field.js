...
KarmaFieldsAlpha.field.table = class extends KarmaFieldsAlpha.field.form {
  ...
  getPage() {
    ...
    content = this.getParam("page").toNumber() || 1;
  ...
  getPpp() { // -> may cause problem if need loading !
    ...
    content = this.getParam("ppp").toNumber() || 100;
  ...
  *add(length = 1) {
  ...
  // async *addFull(index = undefined, length = 1) {
  //
  //   // let set = this.getSet();
  //
  //   while (this.loading) {
  ...
  //   }
  //
  //   this.ids.splice(index, 0, ...tokens);
  //
  ...
  //
  //   // this.doTask("create", tokens, params);
  //
  //
  //
  // }
  ...
  *delete(index, length) {
    ...
    if (length) {
      ...
      // this.ids = [...this.ids];
      //
      // const removedIds = this.ids.splice(index, length);
      //
      // for (let id of removedItems) {
      //
      ...
      for (let driver in delta) {
        ...
        for (let id in delta[driver]) {
          ...
          for (let key in delta[driver][id]) {
            ...
            if (this.isValueModified(delta[driver][id], driver, id, key)) {
...
KarmaFieldsAlpha.field.gridField = class extends KarmaFieldsAlpha.field {
  ...
  *genChildren() {
  ...
  paste(value) {
    ...
    const index = this.querySelection("index") || 0;
    const length = this.querySelection("length") || 0;
  ...
  *import(grid, index, length, columns = this.getExportableColumns()) {
    ...
    if (array.length < length) {
      ...
      yield* this.parent.remove(index + array.length, length - array.length);

    } else if (array.length > length) {

      yield* this.parent.add(index + length, array.length - length);
  ...
  *buildRows() {
    ...
    const offset = (this.getPage().toNumber() - 1)*this.getPpp().toNumber();
    const selection = this.getSelection();
    ...
    for (let i = 0; i < length; i++) {
      ...
      for (let j = 0; j < row.resource.children.length; j++) {
        ...
        yield {
          class: "td table-cell",
          init: td => {
            if (row.resource.children[j].style) {
              td.element.style = row.resource.children[j].style;
            }
          },
          update: td => {
  ...
  //       grid.element.style.gridTemplateColumns = this.getColumns().map(resource => resource.width || "auto").join(" ");
  //
