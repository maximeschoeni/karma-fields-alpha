...
KarmaFieldsAlpha.fields.table = class extends KarmaFieldsAlpha.fields.gateway {
  ...
  constructor(...params) {
    ...
    this.controls = this.createChild(this.resource.controls || "controls");
  ...
  build() {
    return {
      ...
      update: async table => {
        ...
        const singleOpen = Boolean(this.grid.modal) && KarmaFieldsAlpha.Nav.has("id");

        if (active) {

          await this.getIds(); // -> fetch query for modal
              ...
              children: [
                // this.buildModal(),
                {
                  ...
                  update: container => {
                    ...
                    if (singleOpen) {
                      container.children = [this.grid.modal.build()];
                    } else {
                      container.children = [];
                    }
                  }
  ...
  //         });
  //
  //         const modalField = rowField.children.find(child => child.resource.type === "modal");
  //
  //         if (modalField) {
  //
  //           single.children = [
...
KarmaFieldsAlpha.fields.table.controls = class extends KarmaFieldsAlpha.fields.group {
  ...
  // static save = class extends KarmaFieldsAlpha.fields.button {
  //
...
KarmaFieldsAlpha.fields.table.header = class extends KarmaFieldsAlpha.fields.group {
...
// KarmaFieldsAlpha.fields.table.pagination = class extends KarmaFieldsAlpha.fields.group {
//
//   constructor(resource) {
//
//     const defaultResource = {
...
//           update: header => {
//             // header.child = this.createChild("header").build();
//             header.child = this.createChild({
//               type: "header",
...
//             const id = KarmaFieldsAlpha.Nav.get("id");
//
//             // const modalContentField = this.parent.grid.createChild({
//             //   key: id,
//             //   type: "row", // type: "field",
//             //   columns: this.resource.children || [],
//             //   id: id
//             // });
//
//             body.child = this.createChild(this.resource.modal.body || "group").build();
//
//             // const field = this.createChild(this.resource.modal || "group");
//             //
//             // const modalField = rowField.children.find(child => child.resource.type === "modal");
//
//           }
...
//
//       // case "title": {
//       //   event.data = await this.parse(this.resource.modal.title);
//       //   break;
//       // }
//
//       default: {
//         const id = KarmaFieldsAlpha.Nav.get("id");
//         event.path = ["content", id, ...event.path];
//
//         await super.dispatch(event);
//       }
//
...
KarmaFieldsAlpha.fields.table.interface = class extends KarmaFieldsAlpha.fields.field {
  ...
  buildGrid(ids, page, ppp, columns) {
    return {
      class: "table grid",
      init: async grid => {
    ...
    build() {
      return {
        ...
        children: [
          ...
          {
            class: "karma-modal-body",
            child: this.createChild(this.resource.modal.body || this.resource.modal.content || "group").build()
