...
import { build } from 'table-template.mjs';
...
class Table extends Form {
  constructor(resource) {
    ...
    this.data.filters = new Group(resource.filters);
    ...
    this.data.orderby = new Field({
      key: "orderby",
      value: resource.orderby
    });
    this.data.order = new Field({
      key: "order",
      value: this.getDefaultOrder() || "asc"
    });
    this.data.page = new Field({
      key: "page",
      value: 1,
      datatype: "number"
    });
    this.data.ppp = new Field({
      key: "ppp",
      value: field.resource.ppp || 50,
      datatype: "number"
    });
    this.data.count = new Field({
      key: "count",
      datatype: "number"
    });
    this.data.ids = new Field({
      key: "ids",
      datatype: "array"
    });

    this.data.header = new Field();
  ...
  query() {
    ...
    return KarmaFieldsAlpha.Form.fetch(field.resource.driver, "querytable", params).then(function(results) {
      field.data.count.setValue(results.count, "set"); // -> set will save history
      field.data.ids.setValue(results.items.map(function(item) {
        return item.id;
      }), "set"); // -> set will save history
  ...
  sync() {
    ...
    let field = this;
    return KarmaFieldsAlpha.Form.update(resource.driver, value).then(function(results) {
  ...
  add() {
    let field = this;
    return KarmaFieldsAlpha.Form.add(resource.driver, this.data.filters.getValue()).then(function(value) {
      field.createRow(value, true);
  ...
  createRow(value, empty) { // empty prevent loading independant values
    ...
    resource.columns.forEach(function(column) {
      if (column.field) {
        ...
        if (value[column.field.key] !== undefined) {
          ...
          cell.setValue(column.field.default || column.field.value || column.field.getDefault()), "set");
        }
  ...
  reorder(column) {
    if (this.data.orderby.getValue() === this.field.key) {
      this.data.order.setValue(this.data.order.getValue() === "asc" ? "desc" : "asc", "change");
    } else {
      this.data.order.setValue(column.order || "asc", "change");
      this.data.orderby.setValue(column.field.key, "change");
    }
    this.data.page.setValue(1, "change");
    return this.query();
  }
  ...
  saveHistory(field, withUpdate) {
    KarmaFieldsAlpha.History.save(targetField, field.resource.driver, withUpdate);
  }
...
//                   //   key: id
//                   // });
//                   let rowField = field.getDescendant(id) || field.createRow({
//                     key: id
//                   });
//
...
//                         children: [
//                           {
//                             class: "button-content karma-icon",
//                             render: null,
//                             init: function(button) {
//                               let icon = KarmaFieldsAlpha.createField({
//                                 type: "icon",
//                                 file: "update.svg"
//                               });
//                               icon.data.load().then(function(result) {
...
//
//                             // empty cache -> todo: move cache to form field
...
build(field) {
  ...
  return {
    ...
    init: function(container) {
      ...
      field.data.header.events.set = function(field) {
        KarmaFieldsAlpha.History.save(field, field.resource.driver);
        // field.saveHistory(targetField);
      };

      // field.events.request = function(currentField) {
      //   currentField.history.save();
      //   return field.query.then(function() {
      //     container.render(true);
      //   });
                  ...
                  field.resource.columns.forEach(function(column, colIndex) {
                    if (column.type === "index") {
                      ...
                      table.children.push({
                        ...
                        update: function(cell) {
                          // cellField.trigger("update");
                          import(column.field.type+".js").then(function(module) {
