...
KarmaFieldsAlpha.fields.MediaLibrary = class extends KarmaFieldsAlpha.fields.table {

  constructor(...params) {
    super(...params);

    this.paramsBuffer = new KarmaFieldsAlpha.Buffer("params", this.resource.driver);
  ...
  getParam(key) {
    return this.paramsBuffer.get(key);
  }

  setParam(key) {
    return this.paramsBuffer.set(value, key);
  }

  hasParam(key) {
    return this.paramsBuffer.has(key);
  }

  getParams(key) {
    return this.paramsBuffer.getObject();
  }
  ...
  getParamString() {
    ...
    if (!this.hasParam("ppp")) {
      this.setParam("ppp", this.getPpp());
    }

    if (!this.hasParam("orderby")) {
      this.setParam("orderby", this.getOrderby());
    }

    if (!this.hasParam("order")) {
      this.setParam("order", this.getOrder());
    }
  ...
  getDefaultOrderby() {
    return this.defaultOrderby || this.resource.children && this.resource.children.some(child => child.key) && this.resource.find.some(child => child.key).key || "default";
  }
