  ...
  *listInlineRangesAt(range) {
  ...
  breakLine(range, shift) {
	...
	getBrick(node) {
		...
		return KarmaFieldsAlpha.Editor.Brick.get(node);

	}

	breakLineBETA(range, shift) {
		...
		const brick = this.getBrick(range.startContainer);

		brick.break(range, shift);
	...
	breakLine(range, shift) {
		...
		// while ((this.isText(node) || this.isInline(node)) && !this.isRoot(node)) {

		while (!this.isBlock(node) && !this.isContainer(node)) {
		...
		if (this.isRoot(node)) {
			...
			if (this.isContainer(node)) { // figure, table, ul, ol
				...
				let contentBefore = range.extractContents();

				// -> clean content before
				while (contentBefore.lastChild && (this.isText(contentBefore.lastChild) && this.isEmpty(contentBefore.lastChild) || this.isInline(contentBefore.lastChild) && this.isEmpty(contentBefore.lastChild) || this.isSingle(contentBefore.lastChild) && contentBefore.lastChild.tagName === "BR")) {

					contentBefore.removeChild(contentBefore.lastChild);
				...
				this.breakBlock(range, node, nodeResource, container, contentBefore, contentAfter, single);
	...
	// breakBlock(range, node, nodeResource, container, contentAfter, single) {
	//
	// 	if (single) {
	//
	// 		let br = document.createElement("br");
  ...
  //       range.collapse(true);
	...
	breakBlock(range, node, nodeResource, container, contentBefore, contentAfter, single) {
      ...
      if (!this.isEmpty(contentAfter)) {
        ...
        range.collapse(true);
			...
			if (!contentBefore.hasChildNodes()) {
				...
				node.appendChild(contentBefore);
	...
	// divBreakBlock(range, node, nodeResource, container, contentBefore, contentAfter, single) {
	//
	// 	if (nodeResource.breakMode === "div") {
	//
	//
	// 		const block = document.createElement("p");
	// 		block.appendChild(document.createElement("br"));
	...
	// 	} else {
	//
	// 		super.breakBlock(range, node, nodeResource, container, contentBefore, contentAfter, single);
	//
	// 	}
	//
	// }

	containerBreakBlock(range, node, nodeResource, container, contentBefore, contentAfter, single) {

		if (nodeResource.breakMode === "container") {
	...
	columnBreakBlock(range, node, nodeResource, container, contentAfter, single) {

		if (!single && node.tagName === "P" && this.isEmpty(node)  && !this.isRoot(container.parentNode) && (node === node.parentNode.firstChild || node === node.parentNode.lastChild && this.isEmpty(contentAfter))) {
			...
			super.breakBlock(range, node, container, contentAfter);
  ...
  // }
	...
	delete(range) {
		...
		const brick = this.getBrick(range.startContainer, range);

		return brick.backwardDelete(range);
